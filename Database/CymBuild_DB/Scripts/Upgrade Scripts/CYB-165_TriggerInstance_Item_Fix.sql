/*
CYB-165 - Trigger-instance ACT/MS invoice requests must always get at least one InvoiceRequestItem.

What this fixes
----------------
1) The hardened trigger-instance creation path now always creates at least one InvoiceRequestItem
   for ACT / MS requests, even when no billable completed activity can be derived.
2) Placeholder items are zero-value and keep the request itemised rather than leaving header-only rows.
3) Item rows are created with SCore.DataObjects rows first, keeping the CymBuild identity rule intact.
4) A backfill section is included for existing broken ACT / MS automated trigger-instance requests.

Notes
-----
- This script intentionally fixes the current hardened path and the per-job generation path.
- The legacy proc [SFin].[InvoiceAutomationRun_EmitRequests_TriggerInstances] still creates
  headers via [SFin].[InvoiceRequestAutomationCreate] only. If anything still calls that proc,
  it should be retired or refactored to use the hardened path below.
*/
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            /* MS: detailed items from completed billable milestone activities */
            INSERT #ItemsToCreate
            (
                  NewItemGuid
                , InvoiceRequestId
            ...
            /* RIBA: existing behaviour retained */
            INSERT #ItemsToCreate
            (
                  NewItemGuid
                , InvoiceRequestId
            ...
            /* Parse ACT / MS requests so we can create a zero-value fallback item when nothing derived. */
            SELECT
                  ir.InvoiceRequestId
                , ir.InvoicingType
                , ir.InstanceKey
            ...
            /* Always ensure at least one ACT / MS item exists, even if the value is zero or no child activity can be derived. */
            INSERT #ItemsToCreate
            (
                  NewItemGuid
                , InvoiceRequestId
            ...
            /* Recalculate ACT / MS placeholder / reconciliation flags after actual items now exist. */
            ;WITH R AS
            (
                SELECT r.ID
                FROM SFin.InvoiceRequests AS r
...
BEGIN
    ...
    /* Fallback placeholder row: always create one ACT / MS item even when nothing else derived. */
    ;WITH Req AS
    (
        SELECT
              r.ID AS InvoiceRequestId
...
/*
Optional one-off backfill for existing broken automated ACT / MS trigger-instance requests.
Review the SELECT first, then run the INSERT block.
*/
...
(
    SELECT
          r.ID AS InvoiceRequestId
        , r.Guid AS InvoiceRequestGuid
        , r.InvoicingType
        , r.SourceGuid AS TriggerInstanceGuid
        , ti.InstanceKey
    FROM SFin.InvoiceRequests AS r
    JOIN SFin.InvoiceScheduleTriggerInstances AS ti
      ON ti.Guid = r.SourceGuid
    ...
    WHERE r.RowStatus NOT IN (0,254)
      ...
      AND r.SourceType = N'TriggerInstance'
      AND r.InvoicingType IN (N'ACT', N'MS')
    GROUP BY r.ID, r.Guid, r.InvoicingType, r.SourceGuid, ti.InstanceKey
    HAVING COUNT(iri.ID) = 0
)
SELECT *
FROM Missing
ORDER BY InvoiceRequestId;
GO

/* Uncomment to backfill
BEGIN TRAN;

IF OBJECT_ID('tempdb..#MissingItemsBackfill') IS NOT NULL DROP TABLE #MissingItemsBackfill;

SELECT
      ItemGuid = NEWID()
    , m.InvoiceRequestId
    , MilestoneId = CASE WHEN m.InvoicingType = N'MS' THEN ms.ID ELSE act.MilestoneID END
    , ActivityId = CASE WHEN m.InvoicingType = N'ACT' THEN act.ID ELSE NULL END
    , Net = CAST(0 AS DECIMAL(19,2))
    , ShortDescription = LEFT(
          CASE
              WHEN m.InvoicingType = N'ACT' THEN ISNULL(NULLIF(act.Title, N''), N'Activity')
              WHEN m.InvoicingType = N'MS'  THEN ISNULL(NULLIF(ms.Description, N''), N'Milestone')
              ELSE N'Invoice item'
          END,
          200
      )
INTO #MissingItemsBackfill
FROM
(
    SELECT
          r.ID AS InvoiceRequestId
    ...
    WHERE r.RowStatus NOT IN (0,254)
      ...
      AND r.SourceType = N'TriggerInstance'
      AND r.InvoicingType IN (N'ACT', N'MS')
    GROUP BY r.ID, r.InvoicingType, ti.InstanceKey
    HAVING COUNT(iri.ID) = 0
) AS m
LEFT JOIN SJob.Activities AS act
  ON act.ID = CASE WHEN m.InvoicingType = N'ACT' AND m.InstanceKey LIKE N'ACT:%'
                   THEN TRY_CONVERT(BIGINT, SUBSTRING(m.InstanceKey, 5, 200))
                   ELSE NULL END
 AND act.RowStatus NOT IN (0,254)
LEFT JOIN SJob.Milestones AS ms
  ON ms.ID = CASE WHEN m.InvoicingType = N'MS' AND m.InstanceKey LIKE N'MS:%'
                  THEN TRY_CONVERT(BIGINT, SUBSTRING(m.InstanceKey, 4, 200))
                  ELSE NULL END
 AND ms.RowStatus NOT IN (0,254);
...
DECLARE cur_backfill CURSOR LOCAL FAST_FORWARD FOR
    SELECT ItemGuid, InvoiceRequestId, MilestoneId, ActivityId, Net, ShortDescription
    FROM #MissingItemsBackfill;

OPEN cur_backfill;
FETCH NEXT FROM cur_backfill INTO @ItemGuid, @InvoiceRequestId, @MilestoneId, @ActivityId, @Net, @ShortDescription;

...
BEGIN
    ...
    IF (@CompleteStatusGuid IS NULL OR @DeclinedStatusGuid IS NULL)
        THROW 60000, N'Could not resolve one or more enquiry statuses required for reopening.', 1;
    ...
    IF (@LastAppliedStatusGuid IS NULL)
        THROW 60000, N'Could not resolve latest enquiry status.', 1;

    IF (
        @LastAppliedStatusGuid NOT IN
        (
            @CompleteStatusGuid,
            @DeclinedStatusGuid,

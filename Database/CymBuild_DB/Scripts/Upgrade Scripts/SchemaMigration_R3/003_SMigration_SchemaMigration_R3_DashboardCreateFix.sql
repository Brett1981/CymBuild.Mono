/*
    CYB-361/CYB-362 Schema Migration Workbench R3
    Purpose: deployment-safe repair script for the R1 schema migration audit store.
    This script is idempotent and preserves existing rows. It does not execute application schema DDL from the UI.
*/
SET ANSI_NULLS ON;
...
/* Defensive repair for any partially-created R1 rows from early testing. */
IF OBJECT_ID(N'SMigration.Schema_Run', N'U') IS NOT NULL
BEGIN
    UPDATE SMigration.Schema_Run
    SET
        JiraReference = ISNULL(JiraReference, N''),
        ReleaseReference = ISNULL(ReleaseReference, N''),
        DeploymentReference = ISNULL(DeploymentReference, N''),
        Notes = ISNULL(Notes, N''),
        SummaryJson = ISNULL(SummaryJson, N'{}')
    WHERE RowStatus NOT IN (0,254)
      AND
      (
          JiraReference IS NULL
          OR ReleaseReference IS NULL
          OR DeploymentReference IS NULL
          OR Notes IS NULL
          OR SummaryJson IS NULL
      );
END;
GO
...
BEGIN
    UPDATE SMigration.Schema_ExecutionLog
    SET
        Message = ISNULL(Message, N''),
        DetailsJson = ISNULL(DetailsJson, N'{}')
    WHERE RowStatus NOT IN (0,254)
      AND (Message IS NULL OR DetailsJson IS NULL);
END;
GO

IF OBJECT_ID(N'SMigration.Schema_ObjectComparisons', N'U') IS NOT NULL
BEGIN
    UPDATE SMigration.Schema_ObjectComparisons
    SET
        ParentObjectName = ISNULL(ParentObjectName, N''),
        SourceHash = ISNULL(SourceHash, N''),
        TargetHash = ISNULL(TargetHash, N''),
        SourceDefinition = ISNULL(SourceDefinition, N''),
        TargetDefinition = ISNULL(TargetDefinition, N''),
        Notes = ISNULL(Notes, N'')
    WHERE RowStatus NOT IN (0,254)
      AND
      (
          ParentObjectName IS NULL
          OR SourceHash IS NULL
          OR TargetHash IS NULL
          OR SourceDefinition IS NULL
          OR TargetDefinition IS NULL
          OR Notes IS NULL
      );
END;
GO
...
BEGIN
    UPDATE SMigration.Schema_ValidationIssues
    SET
        ObjectType = ISNULL(ObjectType, N''),
        SchemaName = ISNULL(SchemaName, N''),
        ObjectName = ISNULL(ObjectName, N''),
        DetailsJson = ISNULL(DetailsJson, N'{}')
    WHERE RowStatus NOT IN (0,254)
      AND
      (

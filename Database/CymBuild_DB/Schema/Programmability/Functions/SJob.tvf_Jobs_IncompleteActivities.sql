...
(
    ...
    WHERE (j.RowStatus NOT IN (0, 254))
      ...
      -- Incomplete activities exist (unchanged)
      AND EXISTS
      (
          SELECT 1
      ...
      /* --------------------------------------------------------------------
         NOT CANCELLED (latest workflow first, legacy fallback)
         Cancelled WF GUID: 18D8E36B-43BE-4BDE-9D0B-1F34B460AD64
         Legacy fallback: j.JobCancelled is NULL
      -------------------------------------------------------------------- */
      AND
      (
          CASE
              WHEN wf.LatestWorkflowStatusGuid IS NULL
                  THEN CASE WHEN j.JobCancelled IS NULL THEN 1 ELSE 0 END
              ELSE
                  CASE
      ...
      /* --------------------------------------------------------------------
         NOT COMPLETE (latest workflow first, legacy fallback)
      ...
      -- Security + user filter (unchanged)
      AND EXISTS
      (
          SELECT 1

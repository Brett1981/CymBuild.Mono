...
PRINT (N'Create primary key [PK_Icons_ID] on table [SUserInterface].[Icons]')
GO
ALTER TABLE [SUserInterface].[Icons] WITH NOCHECK
  ADD CONSTRAINT [PK_Icons_ID] PRIMARY KEY CLUSTERED ([ID]) ON [METADATA]
GO

PRINT (N'Create unique key [UK_Icons_Guid] on table [SUserInterface].[Icons]')
GO
ALTER TABLE [SUserInterface].[Icons] WITH NOCHECK
  ADD CONSTRAINT [UK_Icons_Guid] UNIQUE ([Guid]) ON [METADATA]
GO

PRINT (N'Create index [IX_UQ_MetricTypes_Name] on table [SUserInterface].[Icons]')
GO
...
PRINT (N'Create trigger [tg_Icons_RecordHistory] on table [SUserInterface].[Icons]')
GO
CREATE TRIGGER [SUserInterface].[tg_Icons_RecordHistory]
   ON  [SUserInterface].[Icons]	
   AFTER INSERT, UPDATE
AS 
BEGIN
	SET NOCOUNT ON;
    ...
    DECLARE	@PreviousValue NVARCHAR(MAX),
			...
			@TableName NVARCHAR(250) = N'Icons',
	...
	BEGIN 
		...
		SELECT	
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'Name', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1377)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'RowStatus', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1374)
			END 
...
PRINT (N'Create foreign key [FK_Icons_RowStatus] on table [SUserInterface].[Icons]')
GO
ALTER TABLE [SUserInterface].[Icons] WITH NOCHECK

PRINT (N'Create table [SJob].[PlanCheckItems]')
GO
...
PRINT (N'Create primary key [PK_PlanCheckItems] on table [SJob].[PlanCheckItems]')
GO
ALTER TABLE [SJob].[PlanCheckItems] WITH NOCHECK
  ADD CONSTRAINT [PK_PlanCheckItems] PRIMARY KEY CLUSTERED ([ID]) WITH (FILLFACTOR = 80)
GO

PRINT (N'Create foreign key [FK_PlanCheckItems_PlanChecks] on table [SJob].[PlanCheckItems]')
GO
ALTER TABLE [SJob].[PlanCheckItems] WITH NOCHECK
  ADD CONSTRAINT [FK_PlanCheckItems_PlanChecks] FOREIGN KEY ([PlanCheckId]) REFERENCES [SJob].[PlanChecks] ([ID])
GO

PRINT (N'Create foreign key [FK_PlanCheckItems_RowStatus] on table [SJob].[PlanCheckItems]')
GO
ALTER TABLE [SJob].[PlanCheckItems] WITH NOCHECK

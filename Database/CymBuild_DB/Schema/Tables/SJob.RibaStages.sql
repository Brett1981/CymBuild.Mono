...
PRINT (N'Create primary key [PK_RIBAStages] on table [SJob].[RibaStages]')
GO
ALTER TABLE [SJob].[RibaStages] WITH NOCHECK
  ADD CONSTRAINT [PK_RIBAStages] PRIMARY KEY CLUSTERED ([ID]) WITH (FILLFACTOR = 80)
GO

PRINT (N'Create index [IX_UQ_RibaStages_Guid] on table [SJob].[RibaStages]')
GO
...
PRINT (N'Create trigger [tg_RibaStages_RecordHistory] on table [SJob].[RibaStages]')
GO
...
PRINT (N'Create foreign key [FK_RibaStages_DataObjects] on table [SJob].[RibaStages]')
GO
ALTER TABLE [SJob].[RibaStages] WITH NOCHECK
  ADD CONSTRAINT [FK_RibaStages_DataObjects] FOREIGN KEY ([Guid]) REFERENCES [SCore].[DataObjects] ([Guid])
GO

PRINT (N'Disable foreign key [FK_RibaStages_DataObjects] on table [SJob].[RibaStages]')
GO
ALTER TABLE [SJob].[RibaStages]
  NOCHECK CONSTRAINT [FK_RibaStages_DataObjects]
GO

PRINT (N'Create foreign key [FK_RibaStages_RowStatus] on table [SJob].[RibaStages]')
GO
ALTER TABLE [SJob].[RibaStages] WITH NOCHECK

...
PRINT (N'Create trigger [tg_Markets_RecordHistory] on table [SCore].[Markets]')
GO
CREATE TRIGGER [SCore].[tg_Markets_RecordHistory]
   ON  [SCore].[Markets]	
   AFTER INSERT, UPDATE
AS 
BEGIN
    ...
    DECLARE	@PreviousValue NVARCHAR(MAX),
			...
			@TableName NVARCHAR(250) = N'Markets',
	...
	BEGIN 
		...
		SELECT	
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'Name', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 2568)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'RowStatus', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 2565)
			END 

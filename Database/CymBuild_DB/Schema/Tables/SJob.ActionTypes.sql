PRINT (N'Create table [SJob].[ActionTypes]')
GO
...
PRINT (N'Create primary key [PK_ActionTypes_ID] on table [SJob].[ActionTypes]')
GO
ALTER TABLE [SJob].[ActionTypes] WITH NOCHECK
  ADD CONSTRAINT [PK_ActionTypes_ID] PRIMARY KEY CLUSTERED ([ID]) WITH (FILLFACTOR = 80)
GO

PRINT (N'Create index [IX_UQ_ActionTypes_Guid] on table [SJob].[ActionTypes]')
GO
...
PRINT (N'Create index [IX_UQ_ActionTypes_Name] on table [SJob].[ActionTypes]')
GO
...
PRINT (N'Create trigger [tg_ActionTypes_RecordHistory] on table [SJob].[ActionTypes]')
GO
CREATE TRIGGER [SJob].[tg_ActionTypes_RecordHistory]
   ON  [SJob].[ActionTypes]	
   AFTER INSERT, UPDATE
AS 
BEGIN
    ...
    DECLARE	@PreviousValue NVARCHAR(MAX),
			...
			@TableName NVARCHAR(250) = N'ActionTypes',
	...
	BEGIN 
		...
		SELECT	
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'Colour', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1575)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'IsActive', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1573)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'Name', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1572)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'RowStatus', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1569)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'SortOrder', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1574)
			END 
...
PRINT (N'Create foreign key [FK_ActionTypes_Guid] on table [SJob].[ActionTypes]')
GO
ALTER TABLE [SJob].[ActionTypes] WITH NOCHECK
  ADD CONSTRAINT [FK_ActionTypes_Guid] FOREIGN KEY ([Guid]) REFERENCES [SCore].[DataObjects] ([Guid])
GO

PRINT (N'Disable foreign key [FK_ActionTypes_Guid] on table [SJob].[ActionTypes]')
GO
ALTER TABLE [SJob].[ActionTypes]
  NOCHECK CONSTRAINT [FK_ActionTypes_Guid]
GO

PRINT (N'Create foreign key [FK_ActionTypes_RowStatus] on table [SJob].[ActionTypes]')
GO
ALTER TABLE [SJob].[ActionTypes] WITH NOCHECK

...
PRINT (N'Create primary key [PK_ActionPriorities_ID] on table [SJob].[ActionPriorities]')
GO
ALTER TABLE [SJob].[ActionPriorities] WITH NOCHECK
  ADD CONSTRAINT [PK_ActionPriorities_ID] PRIMARY KEY CLUSTERED ([ID]) WITH (FILLFACTOR = 80)
GO

PRINT (N'Create index [IX_UQ_ActionPriorities_Guid] on table [SJob].[ActionPriorities]')
GO
...
PRINT (N'Create index [IX_UQ_ActionPriorities_Name] on table [SJob].[ActionPriorities]')
GO
...
PRINT (N'Create trigger [tg_ActionPriorities_RecordHistory] on table [SJob].[ActionPriorities]')
GO
CREATE TRIGGER [SJob].[tg_ActionPriorities_RecordHistory]
   ON  [SJob].[ActionPriorities]	
   AFTER INSERT, UPDATE
AS 
BEGIN
	SET NOCOUNT ON;
    ...
    DECLARE	@PreviousValue NVARCHAR(MAX),
			...
			@TableName NVARCHAR(250) = N'ActionPriorities',
	...
	BEGIN 
		...
		SELECT	
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'Colour', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1583)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'IsActive', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1581)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'Name', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1580)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'RowStatus', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1577)
			END 
			...
			BEGIN 
				...
				VALUES(1, @SchemaName, @TableName, N'SortOrder', @CurrentInsertedID, @CurrentInsertedGuid, @UserID, @PreviousValue, @NewValue, SYSTEM_USER, 1582)
			END 
...
PRINT (N'Create foreign key [FK_ActionPriorities_Guid] on table [SJob].[ActionPriorities]')
GO
ALTER TABLE [SJob].[ActionPriorities] WITH NOCHECK
  ADD CONSTRAINT [FK_ActionPriorities_Guid] FOREIGN KEY ([Guid]) REFERENCES [SCore].[DataObjects] ([Guid])
GO

PRINT (N'Disable foreign key [FK_ActionPriorities_Guid] on table [SJob].[ActionPriorities]')
GO
ALTER TABLE [SJob].[ActionPriorities]
  NOCHECK CONSTRAINT [FK_ActionPriorities_Guid]
GO

PRINT (N'Create foreign key [FK_ActionPriorities_RowStatus] on table [SJob].[ActionPriorities]')
GO
ALTER TABLE [SJob].[ActionPriorities] WITH NOCHECK

...
GO
PRINT (N'Create view [SSop].[Project_ExtendedInfo]')
GO
...
SELECT
    ...
    pl.ListLabel,
    dc.Guid AS DataClassificationID,
    sc.Guid AS SecurityClassificationID
FROM SSop.Projects AS proj
JOIN SSop.ProjectsList AS pl
    ON pl.ID = proj.ID
LEFT JOIN SCore.DataClassifications AS dc
    ON dc.ID = proj.DataClassificationID
   AND dc.RowStatus NOT IN (0,254)
LEFT JOIN SCore.SecurityClassifications AS sc
    ON sc.ID = proj.SecurityClassificationID
...
(
    ...
    (
        ...
        JOIN SSop.QuoteSectionTotals AS qst
            ON qst.ID = qs.ID
...
(
    SELECT
        ...
        CASE
            WHEN SUM(CASE WHEN j.IsCancelled = 0 THEN 1 ELSE 0 END) > 0
            THEN SUM(CASE WHEN j.IsComplete = 1 THEN 1 ELSE 0 END)
                 / SUM(CASE WHEN j.IsCancelled = 0 THEN 1 ELSE 0 END) * 100
            ELSE 0
        END AS PercentageCompleted,
        CASE
            WHEN SUM(fees.Agreed) > 0
            THEN (
                    SUM(fees.Invoiced)
                    / (
                        SUM(CASE WHEN j.IsCancelled = 0 THEN fees.Agreed ELSE 0 END)
                        + SUM(CASE WHEN j.IsCancelled = 0 THEN fees.Invoiced ELSE 0 END)
                      )
                 ) * 100
            ELSE 0
        END AS PercentageAchievableRevenueAchieved
    FROM SJob.Jobs AS j
    OUTER APPLY
    (
        SELECT

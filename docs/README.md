...
For 26.3 implementation work, prefer `V2FormRenderer`, `V2FieldEditor`, `FilteredDynamicGridViewV2`, and established V2 grid/diagnostic patterns. Use a non-V2 route only when it remains the active production route and preserve full parity while migrating it.
...
## **2.2 Run-Based Metadata Migration and Manifest Governance**

CymBuild 26.3 uses a hybrid, source-controlled metadata model. The main migration engine is SMigration run-based; manifests remain ValidateOnly/governance inputs.

```text
Schema authoring = source-controlled SQL
Metadata migration = SMigration run-based code/SQL
Metadata manifests = ValidateOnly/governance support
...
Grid metadata remains ordered `Grid → View → Columns`. Identity is resolved by stable `Guid`, not copied environment-specific IDs. Every inserted metadata entity that requires platform identity must create its matching `SCore.DataObjects` row atomically.

Relevant implementation paths include `MetadataMigration.razor`, `FormHelper.MetadataMigration.cs`, `CoreService.MetadataMigration.cs`, `SQL_CI_Packs_Metadata/database`, `metadata-manifests/v1`, and `libs/Concursus.EF/MetadataManifests/ValidateOnly`.

## **2.3 Schema Migration Workbench and Controlled Deployment**

The workbench is a Development configuration, review and approval surface. It persists source/target comparisons, lightweight selections, selected/default validation, acceptance and audit. It does not execute DDL from the browser.

```text
PWA → FormHelper → gRPC → SMigration persistent plan
→ external controlled runner/worker → source-controlled SQL
```

No saved selection means all deployable rows; saved selection means selected rows only. The R27 runner under `tools/SchemaDeployment` reads `SMigration.SchemaDeploymentPlan_Get`, defaults to dry-run, runs `SCore.PreDeploymentScript`, applies only canonical SQL under `Database/CymBuild_DB/Schema`, runs `SCore.PostDeploymentScript`, and writes DataObjects-compliant audit.

A future Run Deployment button must queue a controlled worker/service account through an outbox/deployment queue. It must not grant DDL permissions to a browser session.
...
* Use `RowStatus <> 0 AND RowStatus <> 254` for active rows unless there is a specific reason not to.
...
* Create the business row and corresponding `SCore.DataObjects` row atomically in the same insert operation.
* Use the correct `Guid`, `EntityTypeId`, `RowStatus`, and platform `RowVersion` behavior.
* Do not use insert-then-update identity patterns.
...
* CymBuild must never call Sage directly; use controlled workers/microservices.
* Ensure idempotency.
...
* SMigration run-based staging, validation, identity mapping, selection, preview, apply and audit
* Manifests retained for ValidateOnly/governance
* Applied idempotently and matched by stable `Guid`
* No duplicates and no manual target editing
...
* Schema Migration Workbench and R27 controlled runner
* Run-based Metadata Migration and manifest governance
...
* Is `RowStatus <> 0 AND RowStatus <> 254` used correctly?
...
* Metadata migration is SMigration run-based; manifests remain ValidateOnly/governance inputs.

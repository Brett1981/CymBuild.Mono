...
$provisioner = Join-Path $RepoRoot 'tools\Testing\Initialize-CymBuildSqlTestDatabase.ps1'
...
if (-not (Test-Path -LiteralPath $provisioner -PathType Leaf)) {
    throw "SQL integration test database provisioner not found: $provisioner"
}
if ($ProvisionOnly -and -not $ProvisionDatabase) {
    throw '-ProvisionOnly requires -ProvisionDatabase.'
}
...
if (-not [string]::IsNullOrWhiteSpace($AllowedDatabaseName)) {
    $provisioningArguments['AllowedDatabaseName'] = $AllowedDatabaseName
}

if ($ProvisionDatabase) {
    $provisioningArguments['Apply'] = $true
    & $provisioner @provisioningArguments
}
else {
    $provisioningArguments['VerifyOnly'] = $true
    & $provisioner @provisioningArguments
}

if ($ProvisionOnly) {
    Write-Host "SQL test database '$databaseName' provisioned and verified. Test execution was not requested."
    return
}

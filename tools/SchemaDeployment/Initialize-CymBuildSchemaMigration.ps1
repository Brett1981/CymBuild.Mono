<#
.SYNOPSIS
    CYB-361 R39 controlled Schema Migration workbench and exclusion-policy bootstrap runner.
...
$exclusionsBootstrapSqlPath = Join-Path $RepoRoot "Database\CymBuild_DB\Schema\Migrations\CYB361\SMigration.SchemaExclusions.Bootstrap.sql"
...
if (-not (Test-Path -LiteralPath $exclusionsBootstrapSqlPath -PathType Leaf)) {
    throw "Source-controlled exclusions bootstrap SQL was not found: $exclusionsBootstrapSqlPath"
}
...
(
    VALUES
        ...
        (90, N'Table', N'[SMigration].[Schema_ExcludedObjects]', CONVERT(BIT, CASE WHEN OBJECT_ID(N'[SMigration].[Schema_ExcludedObjects]', N'U') IS NULL THEN 0 ELSE 1 END)),
        (100, N'StoredProcedure', N'[SMigration].[SchemaExcludedObject_Apply]', CONVERT(BIT, CASE WHEN OBJECT_ID(N'[SMigration].[SchemaExcludedObject_Apply]', N'P') IS NULL THEN 0 ELSE 1 END)),
        (110, N'StoredProcedure', N'[SMigration].[SchemaExcludedObjects_List]', CONVERT(BIT, CASE WHEN OBJECT_ID(N'[SMigration].[SchemaExcludedObjects_List]', N'P') IS NULL THEN 0 ELSE 1 END))
) AS required
(
    [SortOrder],
    [ObjectKind],
...
function Write-BootstrapSummary {
    ...
    $summary = [ordered]@{
        ...
        ExclusionsBootstrapSql = $exclusionsBootstrapSqlPath
        ExclusionsBootstrapSqlSha256 = $script:exclusionsSqlHash
...
$exclusionsBootstrapSql = Get-Content -LiteralPath $exclusionsBootstrapSqlPath -Raw
$sqlHash = (Get-FileHash -LiteralPath $bootstrapSqlPath -Algorithm SHA256).Hash.ToLowerInvariant()
$script:exclusionsSqlHash = (Get-FileHash -LiteralPath $exclusionsBootstrapSqlPath -Algorithm SHA256).Hash.ToLowerInvariant()
...
try {
    ...
    Write-Host "CYB-361 R39 Schema Migration bootstrap"
    ...
    Write-Host "Exclusions SQL  : $exclusionsBootstrapSqlPath"
    ...
    if ($missingBefore.Count -eq 0) {
        Write-Host "All required objects are present. Reapplying the idempotent bootstrap to refresh controlled procedure definitions."
    }
    ...
    Invoke-SqlScriptText -Connection $connection -Sql $exclusionsBootstrapSql -Description "Schema Migration exclusions bootstrap" -TimeoutSeconds 900

# CYB-361 Controlled Schema Deployment Runner (R40)
...
| Manual schema deployment runner and first-class constraints | CYB-361 R40 |
| Last independently verified shared-environment baseline | CYB-361 R27, unless later deployment evidence is supplied |

R39/R40 being present in source does not itself prove that those revisions have already been deployed to a shared environment. Retain the exact run, plan, source hashes and verification evidence for the first R40 rehearsal.
...
- [Constraint deployment contract](ConstraintDeployment.md)
...
- can materialise missing canonical Function, View, StoredProcedure, Trigger or Constraint SQL from the accepted declarative snapshot unless `-SkipSourceMaterialization` is supplied;
...
- standalone index changes without a dedicated migration or approved extractor;
...
## Pre/post deployment and constraint order

Apply mode runs the controlled sequence:

1. read-only migration and constraint preflights;
...
4. constraint prepare files, including foreign-key/key ordering;
5. selected non-constraint source-controlled schema objects in plan order;
6. constraint finalisation files, with primary/unique constraints before foreign keys;
7. `SCore.PostDeploymentScript` after success, except for the established `CymBuild_Dev` rule;
8. SMigration execution audit and final run status.

<#
.SYNOPSIS
    CYB-361 R36 manual schema deployment runner.
...
.DESCRIPTION
    ...
    View, StoredProcedure or Trigger is missing its canonical repository file, the dry-run can
    materialize an idempotent CREATE OR ALTER file from the accepted source-definition snapshot.
...
function Resolve-DeploymentItem {
    ...
    if ($objectType -in @("Index", "Constraint")) {
...
function Write-PlanOutputs {
    ...
    $materializedItems | Export-Csv -Path $materializedCsv -NoTypeInformation
...
function Invoke-CymBuildSchemaDeployment {
    ...
    try {
        ...
        Write-Host "CYB-361 R36 schema deployment runner" -ForegroundColor Cyan
        ...
        if ($materializedItems.Count -gt 0) {
            ...
            foreach ($materializedItem in $materializedItems) {
                Write-Host "  $($materializedItem.ObjectType) $($materializedItem.SchemaName).$($materializedItem.ObjectName) -> $($materializedItem.SourceFile)" -ForegroundColor Green
            }

...
ORDER BY r.ID DESC;", cn)
        ...
        ServerCallContext context)
    {
        ...
        try
        {
            ...
            {
                response.PreviewFingerprint = Convert.ToString(reader["PreviewFingerprint"]) ?? string.Empty;
                response.IsAccepted = Convert.ToBoolean(reader["IsAccepted"]);
                response.AcceptedOnUtc = Convert.ToString(reader["AcceptedOnUtc"]) ?? string.Empty;
                response.AcceptedByUserId = Convert.ToInt32(reader["AcceptedByUserId"]);
            }

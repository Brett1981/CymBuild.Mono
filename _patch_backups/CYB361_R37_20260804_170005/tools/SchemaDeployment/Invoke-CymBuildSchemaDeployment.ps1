<#
.SYNOPSIS
    CYB-361 R27 manual schema deployment runner.
...
function Resolve-DeploymentItem {
    ...
    $result.DeploymentMode = if ($objectType -in @("Table", "TableType", "Sequence")) { "CanonicalCreate" } else { "CanonicalAlter" }
    $result.IsSupported = $true
...
function Invoke-CymBuildSchemaDeployment {
    ...
    try {
        ...
        Write-Host "CYB-361 R27 schema deployment runner" -ForegroundColor Cyan
        ...
        if ($WhatIf) {
            Write-Host "Dry-run only. Read-only target preflights completed, including eligibility validation for schema-bound dependencies scheduled for SCore.PreDeploymentScript; no target schema, data, or SMigration audit rows were changed. Re-run with -Apply to deploy." -ForegroundColor Yellow
            return
        }

...
ORDER BY r.ID DESC;", cn)
        ...
        ServerCallContext context)
    {
        ...
        try
        {
            ...
            if (!run.RunStatus.Equals(SchemaMigrationValidated, StringComparison.OrdinalIgnoreCase))
            {
                return new SchemaMigrationOperationResponse
                {
                    Success = false,
                    Message = "Schema review cannot be completed until the current selected deployment plan has been validated."
                };
            }
...
WHERE Guid = @RunGuid
  ...
  AND RowStatus <> 254;", cn, tx)
    ...
    private static List<SchemaValidationDraft> BuildSchemaValidationIssues(IEnumerable<SchemaMigrationObjectComparisonRow> rows)
    {
        ...
        {
            var unsupportedStandaloneType = row.ObjectType is "Constraint" or "Index" or "Trigger";
            ...
            var severity = row.DifferenceType switch
            {
                ...
                _ when unsupportedStandaloneType || requiresDedicatedMigration => "Warn",
                "Different" => "Info",
            ...
            var message = row.DifferenceType switch
            {
                ...
                _ when unsupportedStandaloneType => $"The current manual runner does not execute standalone {row.ObjectType} rows. Add a dedicated source-controlled idempotent migration script, narrow the accepted selection, or use controlled -AllowPartial deployment.",
                _ when requiresDedicatedMigration => $"Existing {row.ObjectType} definition differs. The current manual runner will not recreate it; provide a dedicated source-controlled, non-destructive migration script.",

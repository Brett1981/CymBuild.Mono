...
BEGIN TRY
    ...
    BEGIN
        /*
            SQL Server does not allow the nullability of a ROWGUIDCOL column to be altered
            while the ROWGUIDCOL property is present. Remove only the column property, alter
            nullability, then restore ROWGUIDCOL inside the same transaction. No row data is
            updated and any failure rolls the complete sequence back.
        */
        ALTER TABLE [SCore].[ObjectSecurity]
            ALTER COLUMN [Guid] DROP ROWGUIDCOL;

        ALTER TABLE [SCore].[ObjectSecurity]
            ALTER COLUMN [Guid] UNIQUEIDENTIFIER NULL;

        ALTER TABLE [SCore].[ObjectSecurity]
            ALTER COLUMN [Guid] ADD ROWGUIDCOL;
    END;
    ...
    BEGIN
        ALTER TABLE [SCore].[ObjectSecurity]
            ALTER COLUMN [ObjectGuid] UNIQUEIDENTIFIER NOT NULL;
    END;

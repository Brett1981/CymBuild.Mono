...
Provision the disposable database through the controlled CymBuild schema and metadata deployment process, then run:

```powershell
.\tools\Testing\Invoke-CymBuildSqlIntegrationTests.ps1 `
    -ConnectionString 'Server=SOC-SQLDEVBRE01\GENERAL;Database=CymBuild_Test_R4;Integrated Security=True;TrustServerCertificate=True'
```

# CYB-361 Manual Schema Deployment Runner (R27)
...
- recognises only active, inspectable schema-bound functions/views whose source-controlled files participate in the existing schema-binding lifecycle when `SCore.PreDeploymentScript` is scheduled to run;
...
It does not recreate the table and does not update or delete business rows. Apply mode executes a read-only preflight before `SCore.PreDeploymentScript`, then repeats the preflight in strict mode after pre-deployment maintenance. The migration is blocked if:
...
3. strict post-pre-deployment preflights confirming approved schema-bound dependencies were actually removed;
4. selected source-controlled schema objects in deployment-plan order;
5. `SCore.PostDeploymentScript`, except when the target database is `CymBuild_Dev`.
...
- blocks full-text, computed-column and unmanaged schema-bound dependencies; read-only preflight may recognise only modules covered by the existing `SCore.SCHEMABINDING` toggle contract, and apply mode verifies that they were removed before altering columns;
...
## R27 managed schema-bound dependency lifecycle

R27 integrates the shared column-alter preflight with the existing CymBuild pre/post deployment lifecycle. The six `SCore.ObjectSecurity` dependencies reported by R26 are real schema-bound views/functions, but their source-controlled definitions use the established `WITH SCHEMABINDING` / `--WITH SCHEMABINDING` marker consumed by `SCore.SCHEMABINDING`. `SCore.PreDeploymentScript` already calls that mechanism with `@Apply = 0`.

The runner now uses a connection-scoped preflight phase flag:

- before pre-deployment, read-only preflight may classify only active, inspectable schema-bound `V`, `IF`, `TF` and `FN` modules as candidates for the existing pre-deployment lifecycle;
- `-SkipPreDeployment` leaves the flag disabled, so those dependencies remain blocking;
- after `SCore.PreDeploymentScript`, the runner clears the flag and repeats each dedicated preflight in strict mode;
- if any schema-bound dependency remains, deployment stops before the table migration and failure recovery attempts `SCore.PostDeploymentScript`;
- the apply helper itself never allows active external schema-bound dependencies and therefore remains independently guarded inside the migration transaction.

This does not dynamically rewrite or deploy view/function definitions. Dry-run establishes eligibility rather than pretending to execute pre-deployment maintenance; apply mode verifies the lifecycle actually removed every dependency before DDL continues.

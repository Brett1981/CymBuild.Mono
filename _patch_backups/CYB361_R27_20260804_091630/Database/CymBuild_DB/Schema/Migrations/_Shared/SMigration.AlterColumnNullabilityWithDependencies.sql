    ...
    */
    ...
    SELECT
        ...
        (
            ...
            (
                ...
                COALESCE
                (
                    QUOTENAME(OBJECT_SCHEMA_NAME(dependencies.referencing_id))
                    + N'.'
                    + QUOTENAME(OBJECT_NAME(dependencies.referencing_id)),
                    N'object_id=' + CONVERT(NVARCHAR(20), dependencies.referencing_id)
                )
            ),
            N', '
        )
    ...
    BEGIN
        DECLARE @ExternalDependencyMessage NVARCHAR(2048) =
            N'CymBuild shared column-alter helper found external schema-bound dependencies on requested columns: '
            + LEFT(@ExternalSchemaBoundDependencies, 1650)
            + N'. Use a dedicated guarded migration.';

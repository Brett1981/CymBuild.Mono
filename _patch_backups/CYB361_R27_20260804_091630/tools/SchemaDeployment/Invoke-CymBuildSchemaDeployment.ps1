<#
.SYNOPSIS
    CYB-361 R26 manual schema deployment runner.
...
function Invoke-CymBuildSchemaDeployment {
    ...
    try {
        ...
        Write-Host "CYB-361 R26 schema deployment runner" -ForegroundColor Cyan
        ...
        if ($WhatIf) {
            Write-Host "Dry-run only. Read-only target preflights completed; no target schema, data, or SMigration audit rows were changed. Re-run with -Apply to deploy." -ForegroundColor Yellow
            return
        }
